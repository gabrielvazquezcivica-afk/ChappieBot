...
export const handler = async (m, { sock, from, isGroup, reply }) => {
  ...
  if (!settings[from]?.customWelcome) return reply('⚠️ No hay mensaje de bienvenida personalizado para borrar')

  delete settings[from].customWelcome
  saveSettings(settings)

  reply('✅ Mensaje de bienvenida restaurado al predeterminado')
}

handler.command = ['deletewelcome']
handler.tags = ['group']
handler.group = true
handler.admin = true
handler.menu = true
